...
// ---------------------------------------------------------------------------
// LanguageSelector is defined at MODULE scope so it is never re-mounted when
// Navbar re-renders (e.g. on every route change or theme toggle).
// ---------------------------------------------------------------------------
const EXT = "png";
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-20">
          <Link to="/" className="flex items-center gap-3 group" aria-label="Ideal Deal Vietnam Home">
            <div
              className="p-1 rounded-sm shadow-sm overflow-hidden flex items-center justify-center"
            ...
            <div className="flex flex-col">
              <span className="font-display font-extrabold text-brandNavy dark:text-white leading-none tracking-tight text-lg uppercase">
                IDEAL <span className="text-primary">DEAL</span>
              </span>
          ...
          <div className="hidden md:flex space-x-8 items-center rtl:space-x-reverse">
            {navLinks.map((link) => (
              <Link
                key={link.name}
                to={link.path}
                className={`text-[12px] font-bold transition-all uppercase tracking-wider ${
                  isActive(link.path)
